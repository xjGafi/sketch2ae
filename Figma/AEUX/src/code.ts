figma.showUI(__html__, { width: 240, height: 238 });
let hasFrameData;
let shapeTree = [];
let imageHashList = [];
let imageBytesList = [];
...
figma.ui.onmessage = (message) => {
    ...
    figma.clientStorage
      ...
      .then((prefs) => {
        if (prefs) {
          ...
          figma.clientStorage
            .setAsync('aeux.prefs', message.defaultPrefs)
            .then(() => {
              figma.ui.postMessage({
                type: 'retPrefs',
                prefs: message.defaultPrefs
              });
            });
  ...
  if (message.type === 'setPrefs') {
    ...
    figma.clientStorage.setAsync('aeux.prefs', message.prefs).then((ret) => {
  ...
  if (message.type === 'exportSelection') {
    ...
    try {
      ...
      figma.ui.postMessage({
        type: 'footerMsg',
        action: 'Layers must be inside of a frame',
        layerCount: null
      });
    }
    ...
    let refImg = null,
      tempGroup,
      parentFrame;
    if (prefs.exportRefImage) {
      // include a reference image with transfer
    ...
    if (rasterizeList.length > 0) {
      ...
      function asyncCollectHashes(id, cb) {
        setTimeout(() => {
          ...
          let compMult = 3;
          let imgScale = Math.min(
            3500 / Math.max(shape.width, shape.height),
            compMult
          ); // limit it to 4000px
...
function nodeToObj(nodes) {
  ...
  if (
    ...
    ((nodes[0].type === 'FRAME' && nodes[0].parent.type === 'PAGE') ||
      ...
      (nodes[0].type === 'COMPONENT' && nodes[0].parent.type === 'PAGE'))
  ) {
    // a frame or a component master outside of a frame is directly selected
    ...
    for (const key in node) {
      try {
        ...
        if (key === 'fills' && element.length > 0) {
          // add image fills to rasterizeList
...
function findFrame(node) {
  ...
  try {
    if (
      (node.type !== 'FRAME' &&
        !(node.type === 'COMPONENT' && node.parent.type === 'PAGE')) ||
  ...
  try {
    ...
    selection.forEach((shape) => {
      if (shape.type == 'GROUP') {
        ...
        shape.exportAsync(options).then((img) => {
          ...
          fillObj.imageTransform = [
            [1, 0, 0],
            [0, 1, 0]
          ];
          fillObj.scaleMode = 'CROP';
          fillObj.type = 'IMAGE';
          (fillObj.scalingFactor = 0.5), delete fillObj.color;
...
async function generateFrameImage() {
  try {
    ...
    parentFrame.exportAsync(options).then((img) => {
