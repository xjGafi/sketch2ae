...
function filterTypes(figmaData, opt_parentFrame, boolType) {
  ...
  layers.forEach((layer) => {
    ...
    if (layer.type == 'GROUP') {
      let prevMask =
        aeuxData.length > 0 ? aeuxData[aeuxData.length - 1].isMask : false; // check if the previous layer is a mask
    ...
    if (layer.type == 'BOOLEAN_OPERATION') {
      ...
      if (layer) {
        // skip if no layers in the compound
        aeuxData.push(layer);
      } else {
        return;
      }
    ...
    if (
      ...
      layer.type == 'FRAME' ||
      layer.type == 'AUTOLAYOUT'
    ) {
      // instances and master symbols
      // console.log('SYMBOL');
      if (layer.rasterize) {
        //
        let rasterizedLayer = getImageFill(layer, parentFrame);
        rasterizedLayer.name = layer.name
          .replace(/^\*\s/, '')
          .replace(/^\*/, ''); // remove astrix
        aeuxData.push(rasterizedLayer);
        rasterizeList.push(layer.id);
        return;
      }

      if (layer.type == 'FRAME' && parentFrame) {
        layer.type = 'AUTOLAYOUT';
      }
...
function getShape(layer, parentFrame, boolType) {
  ...
  if (boolType) {
    frame.x =
      parentFrame.width / 2 +
      (frame.x - layer.width) -
      (parentFrame.width - layer.width) / 2;
    frame.y =
      parentFrame.height / 2 +
      (frame.y - layer.height) -
      (parentFrame.height - layer.height) / 2;
  }
  // if (boolType) {
...
function getText(layer, parentFrame) {
  ...
  function getTextProps(layer) {
    ...
    if (layer.textCase == 'UPPER') {
      text = text.toUpperCase();
    }
    if (layer.textCase == 'LOWER') {
      text = text.toLowerCase();
    }
    if (layer.textCase == 'TITLE') {
...
function getGroup(layer, parentFrame, isMasked) {
  ...
  if (layerData.layers[0] && layerData.layers[0].isMask) {
        ...
        for (var j = 0; j < symbolObj.layers.length; j++) {
          ...
          if (override.symbolOverride) {
            if (currentLayer.id == override.path) {
              // do ids match?
              var overrideSymbol = document.getComponentMasterWithID(
                override.value
              );
  ...
  if (isMultipath) {
    ...
    layerData.layers.forEach((layer) => {
      boolOffset = !boolOffset
        ? { x: layerData.layers[0].frame.x, y: layerData.layers[0].frame.y }
        : boolOffset;
      if (!layer.layers) {
        // lowest level nested shape
...
function getEffects(layer, layerData) {
  ...
  if (layerData.shadow.length == 0) {
    layerData.shadow = null;
  }
  if (layerData.innerShadow.length == 0) {
    layerData.innerShadow = null;
  }
  if (layerData.blur.length == 0) {
    layerData.blur = null;
  }
}
...
function getFrame(layer, parentFrame, constrainFrame) {
  ...
  if (parentFrame) {
    offset = [
      parentFrame.x - parentFrame.width / 2,
      parentFrame.y - parentFrame.height / 2
    ];
  }
  ...
  var x =
    layer.x + Math.cos(Math.atan2(height, width) - angle) * hypot - offset[0];
  var y =
    layer.y + Math.sin(Math.atan2(height, width) - angle) * hypot - offset[1];
  ...
  if (constrainFrame) {
    ...
    if (layer.x + layer.width > frameSize[0]) {
      // off the right edge
    ...
    if (layer.y + layer.height > frameSize[1]) {
      // off the bottom edge
...
function storeArtboard(data) {
  ...
  if (!bgColor || bgColor[3] < 0.0000001) {
    bgColor = [1, 1, 1, 1];
  }
  ...
  for (var i = 0; i < fills.length; i++) {
    ...
    if (fill.visible !== false) {
      ...
      if (fillType[0] > 0) {
        ...
        let points = {};
        if (gradType == 2) {
          let radialPoints = extractRadialOrDiamondGradientParams(
            layer.width,
            layer.height,
          ...
          points = extractLinearGradientParamsFromTransform(
            layer.width,
            layer.height,
            fill.gradientTransform
          );
        ...
        fillObj = {
          ...
          startPoint: [
            points.start[0] - layer.width / 2,
            points.start[1] - layer.height / 2
          ],
          endPoint: [
            points.end[0] - layer.width / 2,
            points.end[1] - layer.height / 2
          ],
          gradType: gradType,
...
function getShapeType(layer) {
  ...
  if (
    ...
    allEqual([
      layer.topLeftRadius,
      layer.topRightRadius,
      layer.bottomLeftRadius,
      layer.bottomRightRadius
    ])
  ) {
    return 'Rect';
  }
...
function colorObjToArray(colorObj) {
  ...
  try {
    ...
    if (c.opacity !== undefined) {
      alpha = c.opacity;
    } else if (c.color.a !== undefined) {
      alpha = c.color.a;
    }

    if (c.color.r === undefined) {
      return null;
    }
...
function getPath(layer, bounding, type) {
  ...
  var pathStr, pathObj;
  if (layer.vectorPaths && layer.vectorPaths.length < 2) {
    // find an individual path
...
function parseSvg(str, transformed) {
  ...
  pathObj.inTangents.splice(
    0,
    0,
    pathObj.inTangents.splice(pathObj.inTangents.length - 1, 1)[0]
  );
  ...
  if (shouldClosePath) {
    ...
    if (
      pathObj.points[0][0] == pathObj.points[pathObj.points.length - 1][0] &&
